...
adata_concat.obs.rename(columns={'slice_id': 'new_batch'}, inplace=True)

adata.obs['Cluster'] = adata.obs['Cluster'].astype('category')
adata.obs.rename(columns={'layer': 'ground_truth'}, inplace=True)

import anndata as ad 
adata.write("../results_dlpfc1/DLPFC_7374_adata.h5ad")
adata = ad.read_h5ad("../results_dlpfc1/DLPFC_7374_adata.h5ad")
...
# # sc.pl.umap(adata, color='new_batch', title='corrected', ax=ax_list[0], show=False)
# # sc.pl.umap(adata, color='ground_truth', title='celltype', ax=ax_list[1], show=False)
# # sc.pl.umap(adata, color='Cluster', title='clusters_GM', ax=ax_list[2], show=False)
# # plt.tight_layout(w_pad=0.05)
# # plt.savefig('../results_dlpfc1/umap_comparison_DLPFC_7374.png')
# # import scib

# # adata.obs["ground_truth"] = adata.obs["ground_truth"].astype("category")
# # adata.obs["new_batch"] = adata.obs["new_batch"].astype("category")

# # sc.pp.neighbors(adata, use_rep="latent")
# # scib.me.graph_connectivity(adata, label_key="ground_truth")
# # scib.me.ilisi_graph(adata, batch_key="new_batch", type_="embed", use_rep="latent")
# # scib.me.kBET(adata, batch_key="new_batch", label_key="ground_truth", type_="embed", embed="latent")
# # scib.me.kBET(adata, batch_key="new_batch", label_key="ground_truth", type_="knn")
# # scib.me.silhouette_batch(adata, batch_key="new_batch", label_key="ground_truth", embed="latent")
