...
# ###如果后面出错 加载模型 
# model_file=('../results/model/SPIRAL_embed_1024_151673_151669_151507.pt')
# model = SPII.model
...
ann.obs["sample_name"] = ann.obs["sample_name"].astype('category')
ann.obs["batch_name"] = ann.obs["sample_name"].copy()

ann.write('../results/spiral_Sample_DLPFC_all.h5ad')
ann = sc.read_h5ad('../results/spiral_Sample_DLPFC_all.h5ad')
...
# # # sc.pl.umap(ann, color='batch_name', ax=axs[0], title='uncorrect', show=False)
# # sc.pp.neighbors(ann,use_rep='spiral',random_state=666)
# # sc.tl.umap(ann,random_state=666)
# # sc.pl.umap(ann, color='batch_name', ax=axs[0], title='corrected', show=False)
# # sc.pl.umap(ann, color='celltype', ax=axs[1], title='Cell Type', show=False)
# # sc.pl.umap(ann, color='mclust', ax=axs[2],title='Clustered UMAP', show=False)

# # plt.tight_layout()
# # plt.savefig('../results/umap_comparison_DLPFC_all.png')

# ##########改文件名
# import os
# files = os.listdir('.')
# prefix_to_remove = '151507-151508-151509-151510-151669-151670-151671-151672-151673-151674-151675-151676_'
# for file in files:
#     if file.startswith(prefix_to_remove):
#         new_name = file.replace(prefix_to_remove, '', 1)  # 只替换文件名前面的前缀
...
# # scib.me.ilisi_graph(ann, batch_key="batch_name", type_="embed", use_rep="spiral")
# # scib.me.kBET(ann, batch_key="batch_name", label_key="celltype", type_="embed", embed="spiral")
# # scib.me.silhouette_batch(ann, batch_key="batch_name", label_key="celltype", embed="spiral")
