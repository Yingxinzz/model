...
model_file = ('../results/model/SPIRAL_embed_1024_151507_151508_151509_151510.pt')
model = SPII.model
...
embed_new_df = pd.concat([
    ...
    embed_df.iloc[:, znoise_dim:]], axis=1)
...
ann = sc.read_h5ad('../results/spiral_Sample3_DLPFC.h5ad')
...
# # sc.tl.umap(ann,random_state=666)
# # sc.pl.umap(ann, color='batch', ax=axs[0], title='correct', show=False)
# # sc.pl.umap(ann, color='celltype', ax=axs[1], title='celltype', show=False)
# # sc.pl.umap(ann, color='mclust', ax=axs[2],title='Cluster_mclust', show=False)
# # plt.tight_layout()
# # plt.savefig('../results/umap_comparison3_DLPFC.png')

# # import scib
# # ann.obs["batch"] = ann.obs["batch"].astype("category")
# # sc.pp.neighbors(ann, use_rep="spiral")
...
# # scib.me.silhouette_batch(ann, batch_key="batch", label_key="celltype", embed="spiral")
