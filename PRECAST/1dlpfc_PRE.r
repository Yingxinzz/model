...
library(Seurat)
library(PRECAST)
seuList <- readRDS("results/1dlpfc_seuList4.RDS")
...
saveRDS(PRECASTObj, file = "results/PRECASTObj.rds")
...
seuInt=readRDS("results/seuInt.rds")
#######显示聚类的空间散点图
...
p12
ggsave("cluster-.png", plot = p12, width = 8, height = 6, dpi = 600)
library(Seurat)
library(ggplot2)
...
ggsave("batch_cluster.png", plot = combined_plot, width = 20, height = 8)
...
adata = sc.read_h5ad('results/1dlpfc_seuInt_with_all_spatial.h5ad')
print(adata.obsm.keys())

adata.write('results/1dlpfc_seuInt.h5ad')
adata = sc.read_h5ad('results/1dlpfc_seuInt.h5ad')
...
# # plt.savefig('results/umap_comparison_DLPFC1.png')
