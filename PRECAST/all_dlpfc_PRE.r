...
name_ID4 <- as.character(c(151673,151669,151507))
...
#     seuList[[i]] <- seu1
# }
saveRDS(seuList, file = "results/dlpfc_all_seuList4.RDS")
library(Seurat)
library(PRECAST)
seuList <- readRDS("results/dlpfc_all_seuList4.RDS")
...
# 保存 PRECASTObj 对象到文件
saveRDS(PRECASTObj, file = "results/PRECASTObj_all.rds")
...
seuInt
# 创建样本名称和对应的分组
sample_group <- rep(NA, length(seuInt$batch))
sample_group[seuInt$batch %in% c("1")] <- "sample1"
sample_group[seuInt$batch %in% c("2")] <- "sample2"
sample_group[seuInt$batch %in% c("3")] <- "sample3"
seuInt$sample_group <- sample_group
head(seuInt@meta.data)
saveRDS(seuInt, file = "results/seuInt_all.rds")
seuInt=readRDS("results/seuInt_all.rds")
...
p12
ggsave("cluster-all.png", plot = p12, width = 8, height = 6, dpi = 600)
...
ggsave("batch_cluster_all.png", plot = combined_plot, width = 20, height = 8)
...
zellkonverter::writeH5AD(sce, "results/dlpfc_all_seuInt.h5ad")
...
import pandas as pd
import os
adata = sc.read_h5ad('results/dlpfc_all_seuInt.h5ad')
...
adata.write('results/dlpfc_all_seuInt_with_all_spatial.h5ad')
adata = sc.read_h5ad('results/dlpfc_all_seuInt_with_all_spatial.h5ad')
print(adata.obsm.keys())

adata.write('results/dlpfc_all_seuInt.h5ad')
adata = sc.read_h5ad('results/dlpfc_all_seuInt.h5ad')
...
# # sc.tl.umap(adata,random_state=666)
# # sc.pl.umap(adata, color='sample_group', ax=ax_list[0], title='Batch corrected', show=False)
...
# # scib.me.ilisi_graph(adata, batch_key="batch", type_="embed", use_rep="PRECAST")
# # scib.me.kBET(adata, batch_key="batch", label_key="Ground_Truth", type_="embed", embed="PRECAST")
# # scib.me.silhouette_batch(adata, batch_key="batch", label_key="Ground_Truth", embed="PRECAST")
